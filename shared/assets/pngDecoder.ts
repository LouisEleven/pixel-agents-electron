...
function readPngSync(pngBuffer: Buffer): PngType {
  const loadRequire = new Function('return require') as () => NodeRequire;
  const pngModule = loadRequire()('pngjs') as typeof import('pngjs');
  return pngModule.PNG.sync.read(pngBuffer);
}
...
export function pngToSpriteData(pngBuffer: Buffer, width: number, height: number): string[][] {
  try {
    const png = readPngSync(pngBuffer);
