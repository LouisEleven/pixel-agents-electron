...
export class PixelAgentsViewProvider implements vscode.WebviewViewProvider {
  ...
  setAgentName(agentId: number, name: string): void {
    const agent = this.agents.get(agentId);
    if (!agent) return;
    agent.customName = name || undefined;
    this.persistAgents();
    this.webview?.postMessage({ type: 'agentNameChanged', id: agentId, name });
  }
  ...
  resolveWebviewView(webviewView: vscode.WebviewView) {
    ...
    webviewView.webview.onDidReceiveMessage(async (message) => {
      if (message.type === 'openClaude') {
        ...
        this.setAgentName(message.id as number, message.name as string);
        ...
        // Send existing agents (with restored customNames) to webview
