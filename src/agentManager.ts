...
export async function launchNewTerminal(
  ...
  const terminal = vscode.window.createTerminal({
    name: `${TERMINAL_NAME_PREFIX} ${folderName ?? `#${idx}`}`,
    cwd,
    env: folderName ? { CLAUDE_AGENT_NAME: folderName } : undefined,
  });
  terminal.show();

  const sessionId = crypto.randomUUID();
  const namePrompt = folderName
    ? ` --append-system-prompt "You are Claude Code Agent '${folderName}'. Remember your name is ${folderName}."`
    : '';
  const claudeCmd = bypassPermissions
    ? `claude --session-id ${sessionId} --dangerously-skip-permissions${namePrompt}`
    : `claude --session-id ${sessionId}${namePrompt}`;
  ...
  webview?.postMessage({ type: 'agentCreated', id, folderName, name: agent.customName });
...
export function sendExistingAgents(
  ...
  // Include folderName, customName, and isExternal per agent
  ...
  for (const [id, agent] of agents) {
    ...
    if (agent.customName) {
      customNames[id] = agent.customName;
    }
    if (agent.isExternal) {
  ...
  console.log(
    `[Pixel Agents] sendExistingAgents: agents=${JSON.stringify(agentIds)}, customNames=${JSON.stringify(customNames)}`,
  );
