...
import {
  ...
  CAMERA_DEFAULT_VERTICAL_OFFSET_FRACTION,
  CAMERA_FOLLOW_LERP,
...
export function OfficeCanvas({
  ...
  const getDefaultPan = useCallback((): { x: number; y: number } => {
    const canvas = canvasRef.current;
    if (!canvas) return { x: 0, y: 0 };

    const layout = officeState.getLayout();
    let minCol = layout.cols;
    let maxCol = CAMERA_CONTENT_BOUNDS_EMPTY_FALLBACK;
    let minRow = layout.rows;
    let maxRow = CAMERA_CONTENT_BOUNDS_EMPTY_FALLBACK;
    ...
    const contentCenterX = ((minCol + maxCol + 1) * TILE_SIZE * zoom) / 2;
    const contentCenterY = ((minRow + maxRow + 1) * TILE_SIZE * zoom) / 2;
    const targetCenterX = canvas.width * (0.5 + CAMERA_DEFAULT_HORIZONTAL_OFFSET_FRACTION);
    const targetCenterY = canvas.height * (0.5 + CAMERA_DEFAULT_VERTICAL_OFFSET_FRACTION);

    return clampPan(targetCenterX - contentCenterX, targetCenterY - contentCenterY);
  }, [clampPan, officeState, zoom]);
